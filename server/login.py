...
def login(embeddingModel):
    ...
    # 배열을 단일 압축 포맷 파일로 저장
    savez_compressed('./files/loginface.npz', trainX, trainy)
    print(trainX.shape, trainy.shape)

    newTrainX, trainy = embedding(embeddingModel, './files/loginface.npz')
    #embedding_file_name = 'login-embeddings.npz'
    # 배열을 하나의 압축 포맷 파일로 저장
    savez_compressed('./files/login-embeddings.npz', newTrainX, trainy )
    user = user_check('./files/login-embeddings.npz')
    if(user):
        return user     
    else :
        return 'NULL'
